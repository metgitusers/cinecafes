            ...
            <div class="row">
                         ...
                         <select name="cafe_id" id="cafe_id" class="form-control">
                           <option value="">Select Cafe</option>
                              <?php foreach($this->CI->getCafeList() as $row1){?>
                                 <option value="<?=$row1['cafe_id'];?>" <?=$row1['cafe_id']==$cafe_id?"selected":"";?>><?=$row1['cafe_name']."-".$row1['cafe_place'];?></option>
                              <?php } ?>
                         </select>
                     </div>
                  </div>
               ...
               <div class="col-md-3">
                  <p style="text-align: left; margin-top: 30px;">                
                     <button style="display: inline-block; width: auto; vertical-align: top;margin-right: 10px;" type="submit"  class="btn btn-primary btn-user btn-block" id="search_btn"><i class="fa fa-search" aria-hidden="true"></i> Search</button>
                     
                     <button style="display: inline-block; width: auto; vertical-align: top;margin-right: 10px;" type="button" onclick="location.href='<?php echo base_url('admin/room');?>';" class="btn btn-primary btn-user ">Reset</button>
                  </p>
               </div>
            </div>
         </form>
               ...
               <tbody>
                  ...
                  <tr>
                     ...
                     <td><?php if(!empty($row['description'])){ echo substr($row['description'],0,20); }?>
                        <?php if(strlen($row['description'])>20){ echo "...";}?>
                     </td>
