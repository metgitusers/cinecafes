 ...
 class Mcommon extends CI_Model {
  ...
  //echo $this->db->last_query();
    ...
    public function user_check($table,$email, $password) {
     ...
     public function admin_login_check($email,$password,$role_id) {

        $this->db->select('user.*');
        $this->db->from('user');
        $this->db->join(' master_role',' master_role.role_id=user.role_id');
        $this->db->where(array('email' => $email, 'password' => $password, 'user.role_id!=' => 0));
...
//echo $this->db->last_query();exit;
    ...
    */
    ...
    public function check_email_id_exist($email,$user_id){
