...
class Reservation extends MY_Controller
{
	...
	public function edit($reservation_id){
		...
		$data 			= array();
		//$condition['user.user_id']			=$reservation_id;
		$data['cafe_list'] = $this->mcommon->getDetails('master_cafe', array('status' => '1', 'is_delete' => '0'));

        $data['media_list'] = $this->mcommon->getDetails('master_media', array('status' => '1', 'is_delete' => '0'));
        $data['member_list'] = $this->Mmembership->getMembershipDetails('1');

        $data['user_list'] = $this->mcommon->select('user', ['status' => 1, 'is_delete' => 0, 'role_id !=' => 1], '*');
		
		//$data['package']	= $this->mpackage->get_package_list();
		$member_data=$this->mreservation->getreservationById($reservation_id);
		//print_r($member_data);die;
		$data['reservation']	= $member_data;
		
		$data['content'] 	= 'admin/reservation/add';
	...
	public function UpdateReservation($reservation_id){
        ...
        $cafe_place       = $this->mcommon->getRow("master_cafe", array('cafe_id'=>$this->input->post('cafe_id')))['reservation_prefix'];
        ...
        if ($this->form_validation->run() == false) {
            ...
            //echo '<pre>';print_r($insrtarry);die;
            //  $reservation_id = $this->mapi->insert('reservation', $insrtarry);
			$condition	= array('reservation_id'=>$reservation_id);
		    $this->mcommon->update('reservation',$condition,$insrtarry);
