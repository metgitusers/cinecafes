    ...
    if ($this->checkHttpMethods($this->http_methods[0])) {
      if (sizeof($ap)) {
          ...
          if(!isset($ap['search_key']) && empty($ap['search_key'])){
            ...
            $this->db->or_like('fi.price', $ap['search_key']);
          ...
          if($sub_categories){
            foreach($sub_categories as $key=> $val){
              ...
              $join = [];
              $join[] = ['table' => 'food_categories fc', 'on' => 'fc.food_category_id = fi.category_id', 'type' => 'left'];
              $val->items = $this->mcommon->select('food_items fi', ['fi.sub_category_id'=> $val->food_category_id, 'fi.status'=> 1], 'fi.*, fc.food_category_id, fc.slug',$sort_column , $sort_order, $join);
  ...
  public function userCheckout()
  {
    ...
    if ($this->checkHttpMethods($this->http_methods[0])) {
      if (sizeof($ap)) {
          ...
          $this->db->where(['member_id'=>$ap['member_id'], 'order_status'=> 0]);
          ...
          if($order_id){
            ...
            unset($default_address['member_id']);
  ...
  public function order()
  {
    ...
    if ($this->checkHttpMethods($this->http_methods[0])) {
      if (sizeof($ap)) {
          ...
          //update coupon
          $this->mcommon->update('food_apply_coupon', ['member_id'=> $ap['member_id']], ['applied_status'=> 1]);
        ...
        /****************** Send password to the member ****************************/
          ...
          $params['subject']  =   'Club Fenicia - Food ordered successfully done';                             
          ...
          $msg                =   registration_mail($params);
              ...
              $message  = "Your food ordered received. Thank you for ordering with us.";
  ...
  public function getOrderedHistory()
  {
    ...
    if ($this->checkHttpMethods($this->http_methods[0])) {
      if (sizeof($ap)) {
          ...
          //get orderedList
          $where = array('member_id'=>$ap['member_id'], 'order_status'=> 1);
          if(isset($ap['order_id'])){
            $where['food_order_id'] = $ap['order_id'];
          }
          $orders = $this->mcommon->select('food_orders', $where, 'food_orders.*', 'food_order_id', 'DESC');
          $orderedList = [];
  ...
  public function getFrontendOrders()
  {
    ...
    if ($this->checkHttpMethods($this->http_methods[0])) {
      if (sizeof($ap)) {        
          ...
          //$categories = $this->mcommon->getDetails('food_categories', $where);
          $join[] = ['table' => 'master_member mm', 'on' => 'mm.member_id = fo.member_id', 'type' => 'left'];
          $items = $this->mcommon->select('food_orders fo', $where, 'fo.*, mm.first_name, mm.last_name', 'fo.food_order_id', 'DESC', $join);
